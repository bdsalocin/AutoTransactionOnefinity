  ...
  const connectWallet = async () => {
    if (web3) {
      ...
      try {
        ...
        // Vérification du réseau et demande de changement si nécessaire
        if (chainId !== CHAIN_ID) {
          try {
            // Demande de basculer vers le OneFinity Testnet
            await window.ethereum.request({
              method: "wallet_switchEthereumChain",
              params: [{ chainId: `0x${CHAIN_ID.toString(16)}` }],
            });
            message.success("Switched to OneFinity Testnet!");
          } catch (switchError) {
            // Si le réseau n'est pas ajouté à MetaMask, demander de l'ajouter
            if (switchError.code === 4902) {
              try {
                await addNetwork(); // Appel à la fonction addNetwork pour ajouter le réseau
              } catch (addError) {
                message.error("Failed to add OneFinity Testnet");
              }
            } else {
              message.error("Failed to switch to OneFinity Testnet");
            }
          }
        }
